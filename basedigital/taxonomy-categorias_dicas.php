...
$thisTerm = $term->taxonomy . '_' . $term->term_id;
...
<div class="slider-dicas">
	...
	foreach ( get_field( 'slider', $idSlider ) as $item ) {
		if ( ! wp_is_mobile() ) {
			?>
            <div class="item" style="background-image: url(<?php echo get_field('imagem_desktop', $thisTerm)['url']; ?>);">
                <div class="info">
                    <h1><?php echo get_field('titulo', $thisTerm); ?></h1>
                    <div class="texto">
						<?php echo get_field('descricao', $thisTerm); ?>
                    </div>
                </div>
            </div>
			<?php
			...
			?>
            <div class="item" style="background-image: url(<?php echo get_field('imagem_mobile', $thisTerm)['url']; ?>);">
                <div class="info">
                    <img src="<?php echo $item['logo_interna']['url']; ?>" alt="<?php echo $item['logo_interna']['alt']; ?>">
                    <h2><?php echo get_field('titulo', $thisTerm); ?></h2>
                </div>
            </div>
			<?php
		}
    ...
    <aside>
            <div class="categories">
                ...
                <?php
                    ...
                    foreach ( $categorias as $categoria ) {
                        $classact = ($term->term_id == $categoria->term_id || $categoria->term_id === 45 && $term->term_id === 36 || $categoria->term_id === 43 && $term->term_id === 37) ? 'active' : 'dflt';
                        ?>
        ...
        foreach($values as $value)
        {
            ?>
                <div class="product_related">
                    ...
                    <a href="<?php echo get_permalink($value->ID); ?>" class="btn-dark white" data-action="conheca-<?php echo sanitize_title($value->post_title); ?>" data-category="<?php echo get_page_uri( $value->ID ); ?>" data-label="">CONHEÇA</a>
