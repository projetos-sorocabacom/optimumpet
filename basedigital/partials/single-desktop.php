...
<div class="container-detail-product">
    ...
    <div class="box-4-right">
        ...
        <div class="slider peso" data-action="Peso" data-category="quantidade-ideal" data-label="">
            <span>Peso</span>
            <input type="range" min="1" max="<?php echo count($output_opcao_1); ?>" step="1" class="peso" value="1"/>
        </div>

        <?php if ($quantidade[0]["opcao_2"] != "") { ?>
            <div class="slider idade" data-action="idade" data-category="quantidade-ideal" data-label="">
                <span>Idade</span>
                <input type="range" min="1" max="<?php echo count($output_opcao_2); ?>" step="1" class="age" value="1"/>
            </div>
        <?php } ?>

        <p class="resposta">Seu <?php echo $animal; ?> deve comer: <label><span class="strong"></span></label></p>

        <p class="importante">*IMPORTANTE: As quantidades indicadas são calculadas para um pet saudável, bem nutrido e não gestante/lactante. As orientações dos guias alimentares são um ponto de partida, e o pet pode precisar de quantidades maiores de alimento <?php 
        if($categorias[1]->slug == "racao-umida"){ echo "úmido";}else {echo "seco";}  ?> para alcançar a condição corpórea ideal, dependendo do nível de atividade diária. Para mais informações, consulte o veterinário</p>
    </div>
</div>
...
<script>
    ...
    var sliderage = document.querySelector('input.age');

    if(sliderage){
        rangeSlider.create(sliderage, {
            polyfill: true,     // Boolean, if true, custom markup will be created
            onInit: function () {
                $(".slider.idade .rangeSlider__handle").html("<span class='label'>" + labels_2[0].opcao_2 + "</span>");

                // match1 = matchCalc(quantidades[0].opcao_2, $(".slider.peso .label").text());
                match1 = quantidades.find(e => e.opcao_2 == labels_2[0].opcao_2 && e.opcao_1 == $(".slider.peso .label").text());

                console.log($(".slider.peso .label").text());
                if (match1) {
            ...
            onSlide: function (position, value) {
                $(".slider.idade .rangeSlider__handle").html("<span class='label'>" + labels_2[position - 1].opcao_2 + "</span>");

                // match1 = matchCalc(quantidades[position - 1].opcao_2, $(".slider.peso .label").text());
                match1 = quantidades.find(e => e.opcao_2 == labels_2[position - 1].opcao_2 && e.opcao_1 == $(".slider.peso .label").text());
                if (match1) {
                    if (match1.e_adulto === "1") {
                        $(".resposta span").html("<a href='" + match1.link + "' target='_blank'>Clique aqui para ver seu <?php echo $animal ?> adulto</a>");
                    } else {
                        var resp = match1.resposta.split("*");
                        $(".resposta label .strong").text(resp[0]+"*");
                        $(".resposta label .light").text(resp[1]);
    ...
    // Initialize a new plugin instance for one element or NodeList of elements.
    var sliderpeso = document.querySelector('input.peso');
    rangeSlider.create(sliderpeso, {
        polyfill: true,     // Boolean, if true, custom markup will be created
        onInit: function () {
            $(".slider.peso .rangeSlider__handle").html("<span class='label'>" + quantidades[0].opcao_1 + "</span>");
            
            // match2 = matchCalc($(".slider.idade .label").text(), quantidades[0].opcao_1);
            match2 = quantidades.find(e => e.opcao_1 == labels_1[0].opcao_1 && e.opcao_2 == $(".slider.idade .label").text());
        ...
        onSlide: function (position, value) {
            $(".slider.peso .rangeSlider__handle").html("<span class='label'>" + labels_1[position - 1].opcao_1 + "</span>");
            // match2 = matchCalc($(".slider.idade .label").text(), quantidades[position - 1].opcao_1);
            match2 = quantidades.find(e => e.opcao_1 == labels_1[position - 1].opcao_1 && e.opcao_2 == $(".slider.idade .label").text());
            if (match2) {
                if (match2.e_adulto === "1") {
                    $(".resposta span.strong").html("<a href='" + match2.link + "' target='_blank'>Clique aqui para ver seu <?php echo $animal ?> adulto</a>");
                    $(".resposta span.light").text(' ');
                } else {
                    var resp = match2.resposta.split("*");
                    $(".resposta label .strong").text(resp[0]+"*");
